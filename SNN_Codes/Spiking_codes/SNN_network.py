...
class spiking_neuron:
    ...
    def SNN_setup(self, neurons, dt, time, recurrent, model, wM, wm, n, codify, maximum , minimun):
        '''
            El metodo calcula los atributos:
            max_weigth
            min_weigth
            time_network
            dt
            ...
            neurons   :  topologia de la red
            dt        :  delta de tiempo
            time      :  tiempo que corre la red
            recurrent :  si el modelo tiene recurrencia
            model     :  modelo de neurona
            wM        :  peso maximo
            wm        :  peso minimo
            n         :  learning rate
            codify    :  codificacion
            maximun   :  frecuencia maxima
            minimum   :  frecuencia minima
        '''

        data=[]  # lista con frecuencia maxima con el tamano de la redundancia 
        data1=[] # lista con frecuencia maxima con el tamano de la redundancia
        neur = cp.copy(neurons) 
        neur[0] = self.redundance # cambiando la cantidad de neuronas de entrada por la redundancia en la copia de la red , ya que solo esa cantidad de neuronas de entrada estaran activas a la vez
        self.max_weigth = wM
        self.min_weigth = wm
        ...
        [layers,monitors,net,m_pesos] = self.SNN_architecture(neur,dt,time,recurrent,model,wM,wm,n,ones=True)  # la red con entrada modificada y pesos maximos
        for i in range(neur[0]):
            data.append(maximum)
            data1.append(minimun)
        dato = self.SNN_encoding(data,1)
        dato2 = self.SNN_encoding(data1,1)
        net.run(inputs={layers[0] : dato}, time=time,reward=0) #se corre la red con maxima frecuencia en la red
        #self.print_spikes(None,None)
        h=monitors[len(layers)-1].get("s")
        h=h.sum(dim=0).tolist()[0] # maxima cantidad de spikes en la salida
        net.run(inputs={layers[0] : dato2}, time=time,reward=0) #se corre la red con minima frecuencia en la red
        #self.print_spikes(None,None)
        c=monitors[len(layers)-1].get("s")
        c=c.sum(dim=0).tolist()[0] # minima cantidad de spikes en la salida
        
        [layer,monitors,net,m_pesos] = self.SNN_architecture(neurons,dt,time,recurrent,model,wM,wm,n,ones=False) # red real
    ...
    def train_episode(self, n_data,reward):
        ...
        print("cod_n_dato shape:", cod_n_dato.shape)
        print("cod_n_dato spikes:", cod_n_dato.sum().item())
        ...
        monitor = self.spiking_monitors[1]

        print("OUTPUT SPIKES:", monitor.get("s").sum().item())
